...
query {
  orders(dateStart: "2021-05-01T00:00:00.000Z", dateEnd: "2021-06-01T07:59:59.999Z", dateType: "delivery") {
    index
    fulfillment {
      groups {
        index
  ...
  timecards(dateStart: "2021-04-01T07:00:00.000Z", dateEnd: "2021-05-01T07:00:00.000Z", dateType: "delivery") {
    id
    entries {
      minutes
      code {
...
function App() {
  ...
  useEffect(()=>{
    axios.post('https://api.webduct.com/public/oauth/authorize/password', {
...
console.log("TOTAL TIME: ", totalTime)
  ...
  for(let index in map){
    ...
    let roundedPoundPerHour = Math.round(roundedWeight/roundedShopLabour) || 0
  ...
  fgArray.push({fg: "TOTAL", weight: separator(Math.round(totalWeight)), shopLabour: separator(Math.round(totalLabor)), poundPerHour: separator(Math.round(totalWeight/totalLabor))})
  for(let obj of fgArray2){
    fgArray.push(obj)
  }
  fgArray.push({fg: "TOTAL", weight: separator(Math.round(totalWeight2)), shopLabour: separator(Math.round(totalLabor2)), poundPerHour: separator(Math.round(totalWeight2/totalLabor2))})
console.log(fgArray)
  return(
    <div style={{ maxWidth: '70%' }}>
    <MaterialTable
    title="Rolling Shop Productivity 5/2021"
    icons={tableIcons}
    options={{
      exportAllData: true,
      exportButton: true,
